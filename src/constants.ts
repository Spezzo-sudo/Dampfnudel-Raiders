...
function createPlanet(
  ...
  const name = PLANET_NAMES[Math.floor(rng() * PLANET_NAMES.length) % PLANET_NAMES.length];
  const owner = isOwn ? 'Du' : OWNER_NAMES[Math.floor(rng() * OWNER_NAMES.length) % OWNER_NAMES.length];
  return {
    id: `${systemId}-P${slot}`,
    systemId,
    slot,
    name: `${name}-${slot}`,
    biome,
    isOwn: isOwn ? true : undefined,
    owner: owner === 'Du' ? undefined : owner,
  };
}

/**
 * Deterministically generates a list of systems for the Galaxy v2 mock data set.
 */
export function generateSystems(count: number, seed = 1337): SystemSnapshot[] {
  const rng = seededRandom(seed);
  const systems: SystemSnapshot[] = [];
  const gridSize = Math.ceil(Math.sqrt(count));
  for (let index = 0; index < count; index += 1) {
    const sectorQ = (index % gridSize) - Math.floor(gridSize / 2);
    const sectorR = Math.floor(index / gridSize) - Math.floor(gridSize / 2);
    const systemId = `SYS-${index}`;
    const biome = SYSTEM_BIOMES[Math.floor(rng() * SYSTEM_BIOMES.length) % SYSTEM_BIOMES.length];
    const planetCount = 4 + Math.floor(rng() * 4);
    const planets: Planet[] = [];
    for (let slot = 1; slot <= planetCount; slot += 1) {
      const isOwn = slot === 1 && index % 17 === 0;
      planets.push(createPlanet(rng, systemId, slot, biome, isOwn));
    }
    systems.push({
      id: systemId,
      sectorQ,
...
/**
 * Default snapshot of systems used by the GalaxyView as lazy-loaded data.
 */
export const SYSTEM_SNAPSHOT: SystemSnapshot[] = generateSystems(2500);
